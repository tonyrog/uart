...
static int my_strncasecmp(const char *s1, const char *s2, size_t n)
{
    ...
    for (i=0;i<(int)n-1 && s1[i] && s2[i] && toupper(s1[i]) == toupper(s2[i]);++i)
	;
			   ...
			   int*     statep)       /* Protocol specific state */
{
    ...
    case UART_PB_ASN1: {
        /* UART_PB_ASN1: handles long (4 bytes) or short length format */
        const char* tptr = ptr;
        int length;
        int nn = n;
...
*/
int uart_packet_parse_http(const char* buf, int len, int* statep,
...
int uart_packet_parse_ssl(const char* buf, int len,
			  PacketCallbacks* pcb, void* arg)
{
